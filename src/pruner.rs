...
fn load_live_keys(live_keys_file: &Path) -> anyhow::Result<HashSet<[u8; HASH_LEN]>> {
    ...
    while let Ok(()) = file.read_exact(&mut buf) {
        ...
        if count % 10_000_000 == 0 {
            eprintln!("   📂 Loaded {}M live keys...", count / 1_000_000);
        }
    }
...
pub fn prune_states(
    ...
    // ── Iterar source e copiar apenas keys vivas ──────────────────
    let mut batch = WriteBatch::default();
    let mut nodes_copied: u64 = 0;
    let mut nodes_deleted: u64 = 0;
    let mut total_scanned: u64 = 0;

    let iter = source.iterator(rocksdb::IteratorMode::Start);
    ...
    for item in iter {
        ...
        if batch.len() >= BATCH_SIZE {
            ...
            if !json && nodes_copied % 500_000 == 0 {
                eprintln!(
                    "   📊 Copied {}M nodes, deleted {}M garbage...",
                    nodes_copied / 1_000_000,
                    nodes_deleted / 1_000_000
                );
            }
        }
    }
